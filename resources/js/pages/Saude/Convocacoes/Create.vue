...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Nova', href: '/saude/convocacoes/create' },
];

const form = useForm({
    programa_id: '' as string | number,
    titulo: '',
    data_convocacao: '',
    hora: '',
    local: '',
...
function selecionarTodos() { form.alunos_ids = alunosFiltrados.value.map(a => a.id); }
function limpar() { form.alunos_ids = []; }
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="mx-auto w-full p-4 md:p-6">
            <div class="mb-6 flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-foreground">Nova Convocação</h1>
                    <p class="text-sm text-muted-foreground">Agende uma ação do Núcleo de Saúde e selecione os alunos.</p>
                </div>
            </div>
            ...
            <form @submit.prevent="submit" class="space-y-6">
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <SearchableSelect
                                ...
                                :error="!!form.errors.programa_id"
                            />
                            <p v-if="form.errors.programa_id" class="mt-1 text-xs text-red-500">{{ form.errors.programa_id }}</p>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <DatePicker v-model="form.data_convocacao" :error="!!form.errors.data" />
                        ...
                        <div>
                            ...
                            <input v-model="form.profissional" type="text" placeholder="Nome do médico, dentista, psicólogo..." class="h-10 w-full rounded-lg border border-input bg-background px-3 text-sm outline-none focus:border-primary" />
                        </div>
                        <div class="sm:col-span-2">
                ...
                <div class="rounded-xl border border-border bg-card p-6 shadow-sm">
                    <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
                        <div>
                            <h2 class="text-lg font-semibold text-foreground">Selecionar Alunos</h2>
                            <p class="text-sm text-muted-foreground">{{ form.alunos_ids.length }} aluno(s) selecionado(s)</p>
                        </div>
                        <div class="flex gap-2">
                            <button type="button" @click="selecionarTodos" class="rounded-lg border border-input px-3 py-1.5 text-xs font-medium hover:bg-muted">Selecionar todos (filtrados)</button>
                            <button type="button" @click="limpar" class="rounded-lg border border-input px-3 py-1.5 text-xs font-medium hover:bg-muted">Limpar</button>
                        </div>
                    </div>

                    <div class="relative mb-3">
                        <Search class="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                        <input v-model="buscaAluno" type="text" placeholder="Buscar por nome do aluno, responsável ou ano escolar..." class="h-10 w-full rounded-lg border border-input bg-background pl-9 pr-3 text-sm outline-none focus:border-primary" />
                    </div>

                    <div class="max-h-[420px] overflow-y-auto rounded-lg border border-border">
                        <p v-if="!alunosFiltrados.length" class="p-4 text-center text-sm text-muted-foreground">Nenhum aluno encontrado.</p>
                        <label v-for="a in alunosFiltrados" :key="a.id"
                            class="flex cursor-pointer items-center gap-3 border-b border-border px-3 py-2 text-sm last:border-0"
                            :class="form.alunos_ids.includes(a.id) ? 'bg-primary/10' : 'hover:bg-muted'">
                            <input type="checkbox" :checked="form.alunos_ids.includes(a.id)" @change="toggleAluno(a.id)" class="rounded border-input text-primary focus:ring-primary" />
                            <div class="flex-1">
                    ...
                    <p v-if="form.errors.alunos_ids" class="mt-1 text-xs text-red-500">{{ form.errors.alunos_ids }}</p>
                </div>

                <div class="flex justify-end gap-3">
                    <Link href="/saude/convocacoes" class="rounded-lg border border-border px-6 py-2.5 text-sm font-medium hover:bg-muted">Cancelar</Link>
                    <button type="submit" :disabled="form.processing" class="rounded-lg bg-primary px-6 py-2.5 text-sm font-medium text-primary-foreground shadow-sm hover:bg-primary/90 disabled:opacity-50">
                        {{ form.processing ? 'Salvando...' : 'Criar Convocação' }}
                    </button>
                </div>
            </form>
        </div>
